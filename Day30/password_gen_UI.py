...
def search_web():
    ...
    try:
        with open(file="data.json") as data:
            total_dict = json.load(data)
    except FileNotFoundError as error_message:
        messagebox.showerror(title="No Web Info", message=f"There is no data.{error_message}, please Add web info")
    else:
        if website_name in total_dict:
            # email = total_dict[website_name]["email"]
            # password = total_dict[website_name]["password"]
            # OR
            sub_dict = total_dict[website_name]
            email = sub_dict['email']
            password = sub_dict['password']
            messagebox.showinfo(title=website_name, message=f"Email:{email}\n\nPassword:{password}")
            # print(total_data[web_name])
        else:
            messagebox.showinfo(title="No web info", message=f"{website_name} not saved. Please add website")
    finally:
            data.close()

# ---------------------------- update json ------------------------------- #

def update_json(entry_dict):
    try:
        ...
        with open("data.json", mode="w") as data:
            json.dump(entry_dict, data, indent=5)

    else:
        content.update(entry_dict)
        with open("data.json", mode="w") as new_data:
            json.dump(content, new_data, indent=5)
    finally:
...
generate_password.config(text="Generate Password", command=gen_password, font=("Arial", 13, "normal"), borderwidth=4)
generate_password.grid(row=4, column=3)

add = Button(width=50)
add.config(text="ADD", command=save_password, font=("Arial", 13, "normal"), borderwidth=4)
add.grid(row=5, column=2, columnspan=2)

search = Button()
search.config(text="Search", command=search_web, font=("Arial", 13, "normal"), borderwidth=4)
